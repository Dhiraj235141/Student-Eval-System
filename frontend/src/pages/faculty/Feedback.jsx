...
import { 
  ...
  Heart, Lightbulb, Loader2, CheckCircle2, Shield 
} from 'lucide-react';
...
export default function FacultyFeedback() {
  ...
  const feedbackTypes = [
    { id: 'Suggestion', label: 'Suggestion', icon: Lightbulb, color: 'indigo' },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Feedback sent to administration! 🚀');
      setSubmitted(true);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div className="card overflow-hidden border-none shadow-xl bg-white p-0">
        <div className="bg-gradient-to-r from-indigo-700 to-purple-800 p-8 text-white text-center relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-indigo-100 mt-2 font-medium">Share your insights to improve the academic ecosystem</p>
          </div>
          {/* Abstract background elements */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -translate-y-32 translate-x-32 blur-3xl" />
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-indigo-400/20 rounded-full translate-y-24 -translate-x-12 blur-2xl" />
        </div>

        <div className="p-8 sm:p-10">
          <div className="space-y-8">
            <div>
              <h3 className="font-bold text-gray-800 mb-6 text-lg">What would you like to share?</h3>
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-6 rounded-2xl border-2 transition-all duration-300 relative group
                        ${isSelected 
                          ? `border-indigo-500 bg-indigo-50 ring-4 ring-indigo-500/10 scale-[1.02]` 
                    ...
                    >
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-transform duration-300 group-hover:scale-110
                        ${isSelected ? 'bg-indigo-500 text-white shadow-lg' : `bg-${t.color}-50 text-${t.color}-500`}`}>
                        <Icon size={24} />
                      </div>
                      <span className={`text-sm font-bold ${isSelected ? 'text-indigo-700' : 'text-gray-600'}`}>{t.label}</span>
                      {isSelected && (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input
                    type="text"
                    placeholder="E.g., Suggestion for the Grading System"
                    className="w-full pl-11 pr-4 py-4 bg-gray-50 border-gray-100 border-2 rounded-2xl focus:bg-white focus:border-indigo-500 transition-all outline-none text-gray-700 font-medium"
              ...
              <div className="space-y-2 pt-2">
                ...
                <textarea
                  rows={6}
                  placeholder="Provide detailed feedback or report an issue..."
                  className="w-full p-5 bg-gray-50 border-gray-100 border-2 rounded-2xl focus:bg-white focus:border-indigo-500 transition-all outline-none text-gray-700 font-medium resize-none shadow-inner"
              ...
              <div className="bg-indigo-50/50 rounded-2xl p-5 flex items-center gap-4 border border-indigo-100/50">
                <div className="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 flex-shrink-0">
                  <Shield size={24} className="animate-pulse" />
                </div>
                <div>
                  <p className="text-sm text-indigo-800 font-bold">Faculty Portal Exclusive</p>
                  <p className="text-xs text-indigo-600 font-medium leading-relaxed">
                    This feedback is delivered directly to the system administrators for priority review.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  ...
                  <span className="relative flex items-center justify-center gap-2 text-lg">
                    ...
                    {loading ? 'Sending...' : submitted ? 'Sent Successfully!' : 'Send Feedback'}
                  </span>
                </button>
              </div>
            </form>
