  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        assignments.map(a => (
          <div key={a.id} className={`card border-2 transition-all ${a.isOverdue && !a.submitted ? 'border-red-100' : a.submitted ? 'border-green-100' : 'border-transparent'}`}>
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              {a.score !== undefined && a.score !== null && (
                <div className="flex-shrink-0 text-center bg-blue-50 rounded-xl px-4 py-2">
            ...
            {/* Upload section — only if not submitted and not overdue */}
            {!a.submitted && (
              <div className="border-t border-gray-100 pt-4 space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => uploadPDF(a.id)}
                    ...
                    className="btn-primary px-5 py-2.5 flex items-center gap-2 flex-shrink-0 text-sm">
                    {uploadingId === a.id ? <><Loader size={14} className="animate-spin" />Uploading...</> : 'Submit'}
                  </button>
                </div>
                {a.isOverdue && (
                  <p className="text-xs text-red-500 flex items-center gap-1">
                    <AlertTriangle size={11} />This assignment is overdue. Late submission may be penalized.
                  </p>
                )}
              </div>
            )}
            ...
            {a.submitted && (
              <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
                ...
                {a.score !== undefined && a.score !== null && (
                  <span className="text-sm font-semibold text-blue-600">Score: {a.score}</span>
                )}
                {(a.score === undefined || a.score === null) && (
                  <span className="text-xs text-gray-400">⏳ Awaiting grading</span>
                )}
              </div>
            )}
          </div>
